CREATE TABLE if not exists `user_acount` (
  ...
  `no` varchar (64) NOT NULL  DEFAULT '' COMMENT '用户编码',
  ...
  PRIMARY KEY (`id`),
  unique KEY `phone_uniq_indx` (`bind_phone`),
  unique KEY `login_uniq_indx` (`login_name`)
...
CREATE TABLE if not exists `user_shipping_address` (
  `id` int (20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',
  `no` varchar (64) NOT NULL  DEFAULT '' COMMENT '地址ID',
  `user_no` varchar (64) NOT NULL  DEFAULT '' COMMENT '用户id',
 ...
 CREATE TABLE if not exists `user_shipping_address_tag` (
  `id` int (20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',
  `no` varchar (64) NOT NULL  DEFAULT '' COMMENT '标签id',
  `user_no` varchar (64) NOT NULL  DEFAULT '' COMMENT '用户id',
...
CREATE TABLE if not exists `goods_wares_category` (
  `id` int (20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',
  `category_code` varchar (64) NOT NULL DEFAULT '' COMMENT '编码',
  `parent_id` int (20) NOT NULL  DEFAULT 0 COMMENT '父类型id',
...
CREATE TABLE if not exists `goods_wares_info` (
  `id` int (20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',
  `wares_no` varchar (64) NOT NULL  DEFAULT '' COMMENT '商品编码',
  `wares_simple_name` varchar (128) NOT NULL  DEFAULT '' COMMENT '商品名称简称',
  `wares_name` varchar (128) NOT NULL  DEFAULT '' COMMENT '商品名称',
  `wares_version` int (20) NOT NULL DEFAULT 0 COMMENT '版本',
  `wares_available` tinyint (2) NOT NULL DEFAULT 0 COMMENT '是否可用，标注当前可用版本',
  `category_category` int (20) NOT NULL  DEFAULT 0 COMMENT '类别id',
  `wares_describe` varchar (512) NOT NULL  DEFAULT '' COMMENT '商品描述',
  `create_time` datetime NOT NULL  COMMENT '创建时间',
  `update_time` datetime NOT NULL  COMMENT '修改时间',
  `status` tinyint (2) NOT NULL DEFAULT 0 COMMENT '数据状态，0 正常 ，1 删除',
  PRIMARY KEY (`id`)
...
CREATE TABLE if not exists `goods_wares_image` (
  `id` int (20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',
  `wares_id` int (20) NOT NULL  DEFAULT 0 COMMENT '商品id',
  `path` varchar (128) NOT NULL  DEFAULT '' COMMENT '地址',
  `cdnName` varchar (128) NOT NULL  DEFAULT '' COMMENT 'cdn地址',
  `image_type` int (20) NOT NULL  DEFAULT 0 COMMENT '图片类型',
  `create_time` datetime NOT NULL  COMMENT '创建时间',
  `update_time` datetime NOT NULL  COMMENT '修改时间',
  `status` tinyint (2) NOT NULL DEFAULT 0 COMMENT '数据状态，0 正常 ，1 删除',
  PRIMARY KEY (`id`)
...
CREATE TABLE if not exists `order_info` (
  ...
  `customer_id` varchar (64) NOT NULL  DEFAULT '' COMMENT '顾客编号',
  `shipping_address_id` int (20) NOT NULL  DEFAULT 0 COMMENT '地址id',
